...
**自动刷新（2026-08-24）**：plan 存在且还有 running/pending/blocked 步骤时，
状态栏每 15s 自动重新快照 `plan.json` 并重渲染——执行用时（⌛）、ETA、心跳警告
（⚠HB）实时更新，Subagent 子进程写回的心跳/进度也能即时反映。无活动步骤或
无 plan 时停止刷新，会话结束（session_shutdown）时自动清理定时器。
间隔可用环境变量 `AUTOGOO_STATUS_REFRESH_MS` 覆盖（调试用）。
